...
tilmap.showTIL=function(){ // get image and display it
    ...
    {
        url2='http://quip1.bmi.stonybrook.edu:443/camicroscope/osdCamicroscope.php?tissueId='+tilmap.selTumorTissue.value.replace('.png','')
        if(!tilmap.selTumorTissue.value.match('-')){ // to accommodate Han's new slides
            let id = tilmap.selTumorTissue.value.match(/\d+/)[0]
            url2="http://quip3.bmi.stonybrook.edu/camicroscope/osdCamicroscope.php?tissueId="+id
        }
        caMicrocopeIfr.src=url2
    }
...
tilmap.getRelative = async function(id,xy){ // converts relative to absolute coordinates
    var url='http://quip1.bmi.stonybrook.edu:443/camicroscope/api/Data/getImageInfoByCaseID.php?case_id='+id
